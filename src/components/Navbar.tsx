  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-background/90 backdrop-blur-xl border-b border-white/10 shadow-lg" : "bg-transparent"}`}>
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex items-center justify-between transition-[height] duration-400 ${scrolled ? "h-[64px]" : "h-[84px]"}`}>
          {/* Logo — shrinks slightly on scroll for a tighter sticky header */}
          <AnimatedLogo size={scrolled ? 40 : 48} withWordmark asLink />
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <button
              ...
              className="group flex items-center gap-2 px-3.5 py-2 rounded-full glass text-xs font-semibold text-white/80 hover:text-white hover:bg-white/10 transition-all focus-visible:ring-2 focus-visible:ring-orange-400"
              aria-label="Toggle language"
            >
              <Globe className="w-3.5 h-3.5 group-hover:animate-pulse" />
              <div className="flex items-center gap-1.5">
                <span className={`transition-colors ${lang === "en" ? "text-orange-400 drop-shadow-[0_0_8px_rgba(245,130,32,0.5)]" : "text-white/50"}`}>EN</span>
                <span className="text-white/20">|</span>
                <span className={`transition-colors ${lang === "bn" ? "text-orange-400 drop-shadow-[0_0_8px_rgba(245,130,32,0.5)]" : "text-white/50"}`}>বাং</span>
              </div>
            </button>
          ...
          <div className="md:hidden flex items-center gap-2.5">
            ...
            <button
              onClick={() => setLang(lang === "en" ? "bn" : "en")}
              className="flex items-center gap-1 px-3 py-2 rounded-full glass text-[11px] font-semibold text-white/80 hover:bg-white/10 transition-colors"
            >
              <Globe className="w-3.5 h-3.5" />
              <span className={lang === "en" ? "text-orange-400" : "text-white/60"}>
                {lang === "en" ? "বাং" : "EN"}
              </span>
            </button>
            <button onClick={() => setIsOpen(!isOpen)} aria-label={isOpen ? "Close menu" : "Open menu"} className="w-10 h-10 flex items-center justify-center text-white bg-white/5 rounded-full hover:bg-white/10 transition-colors">
