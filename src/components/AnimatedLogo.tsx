...
export default function AnimatedLogo({ size = 44, withWordmark = false, className = "", asLink = false }: AnimatedLogoProps) {
  const inner = (
    <span
      className="relative inline-flex items-center justify-center transition-transform duration-500 group-hover:scale-[1.04]"
  ...
  if (asLink) {
    return <Link to="/" aria-label="ATDB Trade International — Home" className="inline-flex">{content}</Link>;
  }
  return content;
}
